...
class Wormcog(commands.Cog):
    ...
    async def smartSend(self, ctx, *, content: str = None, embed: discord.Embed = None):
        ...
        if hasattr(ctx.channel, "id") and repo_w.get(ctx.channel.id) is not None:
            await asyncio.get_event_loop().run_until_complete(ctx.send(content=content, embed=embed, delete_after=self.delay()))
        else:
            await asyncio.get_event_loop().run_until_complete(ctx.send(content=content, embed=embed))
