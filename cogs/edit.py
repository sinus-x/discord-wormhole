...
class Edit(wormcog.Wormcog):
    ...
    async def admin(self, ctx: commands.Context):
        ...
        embed.add_field(
            name=f"{p}wormhole open", value="Open wormhole in current channel", inline=False
        )
        embed.add_field(
            name=f"{p}wormhole close", value="Close wormhole in current channel", inline=False
        )
    ...
    async def edittimeout(self, ctx: commands.Context, value: str):
        ...
        self.confSave()
        await self.send(
            "> New time limit for message edit/deletion: **{value} s**", announcement=True
        )
        await self.delete(ctx.message)
